document.addEventListener('DOMContentLoaded', () => {
  ...
  function init_node(node, first)
  {
    ...
    node_elem.querySelector('.title-box').setAttribute('title', (block.title ? `${id_string} - ${block.title}` : id_string));
  ...
  const blocks = {
    'audio-play': {
      ...
      inputs: 1,
      outputs: 0,
      body: bodys.file('File', false, 'audio-play') + bodys.number('Volume', false, 100, 1, 0, 100, true) + bodys.select('Device', false, false, false, true),
      ...
      update: (id, elem, data, set_data, receive, receive_data) => {
        ...
        {
          elem.classList.add('block-init');
          browse_fas(id, 'file', elem.querySelector('.audio-play button'), 'file');
        }
    ...
    'kill-app': {
      ...
      update: (id, elem, data, set_data, receive, receive_data) => {
        ...
        {
          elem.classList.add('block-init');
          browse_fas(id, 'file', elem.querySelector('.kill-app button'), 'program');
        }
      }
    },
    'launch-app': {
