document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof editor.renameNode !== 'function') {
    editor.renameNode = (node_id, new_name) => {
      const node = window.get_node(node_id);
      if (node) {
        node.data.data.block_title = new_name.trim();
        if (!node.data.data.block_title) {
          delete node.data.data.block_title;
        }

        const title_box = node.elem.querySelector('.title-box');
        if (title_box) {
          const block = window.get_block(node.data.type),
            title = title_box.querySelector('span'),
            id_string = `[${node.id}]`;
        ...
        window.set_data(node.id, node.data.data);
      }
    };
  }
  ...
  editor.on('moduleChanged', name => {
    ...
    init();
    if (selector.getValue && selector.getValue() !== name) {
  ...
  document.querySelector('div.rename-block .modal-card-body input[type="text"]').addEventListener('keypress', event => {
    if (event.code === 'Enter') {
      document.querySelector('div.rename-block .is-success').dispatchEvent(new Event('click', { bubbles: true }));
    }
  }, false);
  ...
  document.querySelector('div.edit-module .module-enabled').addEventListener('click', editor.toggleModule, false);
  document.querySelector('div.edit-module .module-disabled').addEventListener('click', editor.toggleModule, false);
  ...
  window.message_event = _event => {
    const { to, event, name, method, property, data } = _event.data;
    if (_event.origin !== 'null' && to === 'interface') {
      if (property === 'config') {
        setTimeout(() => {
          _config = data;
          ...
          if (_config.settings.lateral) {
            document.querySelector('.button.lateral-blocks').dispatchEvent(new Event('click', { bubbles: true }));

            if (_config.settings.toggle) {
              document.querySelector('.show-blocks').dispatchEvent(new Event('click', { bubbles: true }));
            }
          }
      ...
      } else if (property === 'receive' && typeof window.drawflow_receiver === 'function') {
        const { source, name, method, property, separate } = data;
        window.drawflow_receiver(source, name, method, property, data.data, separate);
      } else if (property === 'import') {
        window.import_nodes(window.filter_nodes(data));
      } else if (property === 'toggle-block' && typeof editor.drawflow.drawflow[data.module] !== 'undefined' && typeof editor.drawflow.drawflow[data.module].data[data.id] !== 'undefined') {
        editor.drawflow.drawflow[data.module].data[data.id].data.data.enabled = data.enabled;

        const node = window.get_node(data.id);
        if (node) {
          node.elem.classList.toggle('disabled', !data.enabled);
        }
      } else if (property === 'keyboard') {
        window.drawflow_receiver(false, 'method', property, false, data, _event.data.separate);
      }
    }
  };
